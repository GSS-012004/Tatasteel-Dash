...
import { AfterViewInit, ChangeDetectionStrategy, ChangeDetectorRef, Component, ElementRef, NgZone, OnChanges, OnDestroy, OnInit, Query, SimpleChanges, ViewChild, ViewChildren } from '@angular/core';
...
export class RaViolationsComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    this.zone.run(() => {
      ...
      this.webServer.raLiveInterval = setInterval(() => {
        if (!this.isdate) {
          ...
          this.Subsciption = this.webServer.LiveRAViolationData(this.selectedCameraId).subscribe((Rdata: any) => {
            this.dataFetchStatus = 'success'
            //  this.cd.detectChanges()
  ...
  sliceVD() {
    if (!this.isdate) {
      ...
      this.total = of(this.tempdata.length)
      this.violData = of((this.tempdata.map((div: any) => ({ ...div })).slice((this.page - 1) * this.pageSize, (this.page - 1) * this.pageSize + this.pageSize)))
    }
    if (this.isdate) {
      var table = document.getElementById('dataTable')
      table?.classList.add('loading')
      this.webServer.DatewiseRAViolations(this.fromDate, this.toDate, this.page, this.pageSize, this.selectedCameraId ? this.selectedCameraId : null).subscribe((Response: any) => {
        if (Response.success) {
          table?.classList.remove('loading')
  ...
  Submit() {
    ...
    this.zone.run(() => {
      ...
      this.isdate = true
      this.loading = true
      this.webServer.DatewiseRAViolations(this.fromDate, this.toDate, null, null, this.selectedCameraId ? this.selectedCameraId : null).subscribe((Response: any) => {
        if (Response.success) {
          // this.cd.detectChanges()
  ...
  async submitForm() {
    ...
    this.webServer.CreateRAViolationExcel(body).subscribe((Response: any) => {
      if (Response.success) {
  ...
  GetViolationLength(fromDate: any, toDate: any, cameraName: any) {
    ...
    var length
    this.webServer.DatewiseRAViolations(fromDate, toDate, null, null, cameraName ? cameraName : null, null).subscribe((Response: any) => {
      if (Response.success) {
  ...
  getLatestData() {
    ...
    var table = document.getElementById('dataTable')
    table?.classList.add('loading')
    this.webServer.GetLatestRAData(this.selectedCameraId).subscribe((Rdata: any) => {
      if (Rdata.success) {
        this.isLatest = true
        table?.classList.remove('loading')
  ...
  getCameraList() {
    ...
    this.webServer.GetRACameraDetails().subscribe((data: any) => {
