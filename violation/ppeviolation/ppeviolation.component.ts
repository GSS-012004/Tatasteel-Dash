...
export class PpeviolationComponent implements OnInit,OnDestroy,AfterViewInit {
  ...
  helmetFilterValue:FormControl<Number>=new FormControl<Number>(30,[Validators.pattern(new RegExp('^(?!-)[0-9]+$')),Validators.min(30),])
  vestFilterValue:FormControl<Number>=new FormControl<Number>(30,[Validators.pattern(new RegExp('^(?!-)[0-9]+$')),Validators.min(30)])
  tempdata: any[] = [];
  page: number = 1
  pageSize: number = 30
 ...
 time: any
  ...
  ngOnInit(): void {
    ...
    if (!this.latest || this.isLatest) {
      this.webServer.LivePPEViolationData().subscribe((Rdata: any) => {
        if (Rdata.success) {
  ...
  public dataread() {
    ...
    this.interval = setInterval(() => {
      if (!this.isdate) {
        ...
        this.Subsciption = this.webServer.LivePPEViolationData(this.selectedCameraId, this.selectedViolType).subscribe((Rdata: any) => {
          // console.log(Rdata)
        //    var table = document.getElementById('dataTable')
        // table?.classList.add('loading')
        ...
        if (false) {
          this.webServer.LivePPEViolationData().subscribe((Response: any) => {
            if (!this.latest) {
  ...
  sliceVD() {
    ...
    if (this.isdate) {
      var table = document.getElementById('dataTable')
      table?.classList.add('loading')
      this.webServer.DatewisePPEViolations(this.fromDate, this.toDate, this.page, this.pageSize, this.selectedCameraId ? this.selectedCameraId : null, this.selectedViolType ? this.selectedViolType : null).subscribe((Response: any) => {
        if (Response.success) {
          table?.classList.remove('loading')
  ...
  Submit() {
    ...
    this.isdate = true
    this.loading = true
    this.webServer.DatewisePPEViolations(this.fromDate, this.toDate, null, null, this.selectedCameraId ? this.selectedCameraId : null, this.selectedViolType ? this.selectedViolType : null).subscribe((Response: any) => {
      this.dataFetchStatus='init'
      if (Response.success) {
        if (Response.message.length == 0) {
          this.tempdata = []
  ...
  SetPPEFilters(){
    this.dataFetchStatus='init'
    this.webServer.ChangePPEFiltersData({ppepercentage:{helmet:this.helmetFilterValue.value,vest:this.vestFilterValue.value}}).subscribe((response:any)=>{
       this.dataFetchStatus='success' 
      this.GetPPEFilterValues()
       if( !this.isdatewise){
        this.tempdata=[]
  ...
  GetPPEFilterValues(){
    this.webServer.GetPPEFiltersData().subscribe((response:any)=>{
      if(response.success){
  ...
  async submitForm() {
    ...
    var body = {
      ...
      cameraname: this.selectedItems ? this.selectedItems.data=='all_cameras'?null:this.selectedItems.data : null,
    }
    ...
    const diffInDs = (Difference_In_Time) / (1000 * 3600 * 24)
      ...
      this.webServer.CreatePPEViolationExcel(body).subscribe((Response: any) => {
        if (Response.success) {
  ...
  //-------METHOD TO DOWNLOAD THE EXCEL--------
   GetViolationLength(fromDate:any,toDate:any,cameraName:any,violationType?:any) {
    ...
    var length
    this.webServer.DatewisePPEViolations(fromDate, toDate, null, null, cameraName?cameraName:null,violationType?violationType:null).subscribe((Response: any) => {
      if (Response.success) {
  ...
  GetLatestPPEData() {
    ...
    this.webServer.GetLatestPPEData(this.selectedCameraId).subscribe((Rdata: any) => {
      if (Rdata.success) {
        this.isLatest = true
        table?.classList.remove('loading')
