...
export class CrowdCountViolationsComponent implements OnInit,OnDestroy ,AfterViewInit{
 ...
 this.dropdownSettings = {
  ...
  public dataread() {
    ...
    this.interval = setInterval(() => {
      if (!this.isdate) {
        ...
        this.Subsciption = this.webServer.LiveCCViolationData(this.selectedCameraId, this.selectedViolType).subscribe((Rdata: any) => {
          this.dataFetchStatus='success'
          if (Rdata.success) {
            var response = { ...Rdata }
        ...
        if (false) {
          this.webServer.LiveCCViolationData().subscribe((Response: any) => {
            if (!this.latest) {
  ...
  sliceVD() {
    ...
    if (this.isdate) {
      var table = document.getElementById('dataTable')
      table?.classList.add('loading')
      this.webServer.DatewiseCCViolations(this.fromDate, this.toDate, this.page, this.pageSize, this.selectedCameraId ? this.selectedCameraId : null, this.selectedViolType ? this.selectedViolType : null).subscribe((Response: any) => {
        if (Response.success) {
          table?.classList.remove('loading')
  ...
  Submit() {
    ...
    this.isdate = true
    this.loading = true
    this.webServer.DatewiseCCViolations(this.fromDate, this.toDate, null, null, this.selectedCameraId ? this.selectedCameraId : null, this.selectedViolType ? this.selectedViolType : null).subscribe((Response: any) => {
      this.dataFetchStatus='success'
      if (Response.success) {
        if (Response.message.length == 0) {
          this.tempdata = []
  ...
  async submitForm() {
    ...
    var body = {
     ...
     this.webServer.CreateCCViolationExcel(body).subscribe((Response: any) => {
        if (Response.success) {
  ...
  //-------METHOD TO DOWNLOAD THE EXCEL--------
   GetViolationLength(fromDate:any,toDate:any,cameraName:any,violationType?:any) {
    var length
    this.webServer.DatewiseCCViolations(fromDate, toDate, null, null, cameraName?cameraName:null,violationType?violationType:null).subscribe((Response: any) => {
      if (Response.success) {
  ...
  GetLatestCCData() {
    ...
    table?.classList.add('loading')
    console.log(this.selectedViolType)
    this.webServer.GetLatestCCData(this.selectedCameraId).subscribe((Rdata: any) => {
      if (Rdata.success) {
        this.isLatest = true
        table?.classList.remove('loading')
